-- Repair the UTF-8 Bengali template used by document-review notifications.
...
-- Existing review notifications can be reconstructed from their structured metadata.
UPDATE public.waqf_messages
SET text = 'আপনার “'
  || COALESCE(NULLIF(extra ->> 'fileName', ''), 'ডকুমেন্ট')
  || '” ডকুমেন্টটি পর্যালোচনা করা হয়েছে।'
  || CASE
       WHEN COALESCE(extra ->> 'reviewComment', '') <> ''
         THEN ' · মন্তব্য: ' || (extra ->> 'reviewComment')
       ELSE ''
     END
WHERE role = 'out'
  AND type = 'text'
  AND extra ? 'reviewComment'
  AND (text LIKE '%à¦%' OR text LIKE '%â€œ%' OR text LIKE '%Â·%');

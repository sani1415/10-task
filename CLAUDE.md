...
- Current version: **v191** (last bumped: teacher PWA persistent session until logout)
...
- **Production DB — migration order:**
  ...
  24. `migrations/20260713100000_security_patch_msg_storage_push.sql` — ৩টি হার্ডেনিং: (ক) `madrasa_rel_insert_message` এখন thread + role মালিকানা যাচাই করে (ছাত্র আর শিক্ষক/`out` বা `_bc` মেসেজ, বা অন্য ছাত্রের থ্রেডে মেসেজ জাল করতে পারবে না); (খ) storage `waqf-files`-এ অব্যবহৃত anon `DELETE` + `UPDATE` policy বাদ (আপলোড unique path-এ, ক্লায়েন্ট কখনো object মোছে/overwrite করে না); (গ) `madrasa_rel_save_pwa_subscription`-এ নতুন `p_pin` আর্গ — `id='teacher'` ও ব্যক্তিগত ছাত্র (`id=waqf_id`) slot PIN-gated, `shared_device_*` খোলা (লগইনের আগে, random id)। **ক্লায়েন্ট জোড়া:** `pwa-notify.js` এখন `p_pin` পাঠায়, `remote-sync.js` `getStudentPin()` এক্সপোজ করে — migration আর ক্লায়েন্ট একসাথে deploy করতে হবে।
  25. `migrations/20260713140000_waqf_fortnightly_report.sql` — `waqf_madrasa_config`-এ ৪টি কলাম (`fortnightly_enabled`, `fortnightly_interval_days`, `fortnightly_category_id`, `fortnightly_questions`); নতুন RPC `madrasa_rel_update_fortnightly_config`; `madrasa_rel_student_bootstrap` প্যাচ করা হয়েছে যাতে ছাত্র এই ৪টি ফিল্ড পায় (আগে শুধু madrasa/teacher_name যেত)। **পাক্ষিক বিবরণ লক ফিচার:** শিক্ষকের সেটিংয়ে (teacher.html → «পাক্ষিক বিবরণ লক») enable/disable, ইন্টারভাল (দিন), কোন note category «পাক্ষিক বিবরণ» ধরা হবে, আর ঐচ্ছিক প্রশ্নের তালিকা কনফিগার করা যায় (`API.StudentNotes.getFortnightlyConfig/saveFortnightlyConfig`, remote হলে `RemoteSync.updateFortnightlyConfigRemote`)। চালু থাকলে ছাত্র লগইন করার পর `API.StudentNotes.getFortnightlyStatus(sid)` চেক হয় (student.html → `checkFortnightlyLock()`) — নির্ধারিত ক্যাটাগরিতে শেষ নোটের তারিখ থেকে ইন্টারভাল পার হয়ে গেলে (বা কখনো জমা না দিলে), আলাদা কোনো নতুন স্ক্রিন/ফর্ম না বানিয়ে **existing «নতুন বিবরণ» মোডাল (`dailyNoteModal`)-ই** জোরপূর্বক খোলা হয় (`openDailyNoteModal(cfg, lastDate)`) — ক্যাটাগরি লক করা (নির্ধারিত fortnightly ক্যাটাগরিতে আটকানো, ড্রপডাউন disabled), «বাতিল» বাটন হয়ে যায় «লগ-আউট», আর backdrop/স্কিপ করে বন্ধ করা যায় না (`dailyNoteForced` ফ্ল্যাগ গার্ড করে `closeDailyNoteModal()`)। টিচারের কনফিগার করা প্রশ্নগুলো টেক্সটএরিয়ার placeholder-এ দেখানো হয় (গাইড হিসেবে), কিন্তু answer একটাই সাধারণ লেখা — জমা দিলে স্বাভাবিক `student_notes` হিসেবেই সেভ হয় (নতুন টেবিল লাগেনি) এবং আনলক হয়। (`logout()`-এ `goTab('tasks', true)` — `instant` মোড View Transition পুরোপুরি স্কিপ করে, লগইন/লগআউটের সময় ভুল স্ক্রিন morph হওয়া ঠেকায়।)
  26. `migrations/20260713160000_waqf_student_notes_review.sql` — `waqf_student_notes`-এ `review_status` কলাম (ডিফল্ট `'done'`); `madrasa_rel_upsert_student_note`/`madrasa_rel_delete_student_note` এখন `review_status='done'` হলে ব্লক করে (`RAISE EXCEPTION 'note_locked'`); নতুন RPC `madrasa_rel_mark_note_reviewed` (শিক্ষক-only)। **বিবরণ রিভিউ ওয়ার্কফ্লো:** নতুন বিবরণ সবসময় `'pending'`-এ শুরু হয়; পেন্ডিং অবস্থায় ছাত্র এডিট/ডিলিট করতে পারে (student.html `renderDailyNotes()` — বাটন + «পর্যালোচনাধীন» ব্যাজ দেখায়), শিক্ষক ছাত্রের বিবরণ ভিউতে (teacher.html `renderChatNotes()`) «✅ পর্যালোচনা সম্পন্ন» বাটনে ক্লিক করলে (`markNoteReviewed()` → `API.StudentNotes.markReviewed`) `review_status='done'` হয়ে যায়, এরপর ছাত্র আর এডিট/ডিলিট করতে পারবে না (client + সার্ভার দুই জায়গাতেই `note_locked` গার্ড)। **নিজামুল আওকাত ডিলিট + ডিফ ভিউ (কোনো DB পরিবর্তন লাগেনি):** `api-daily-schedule.js`-এ নতুন `DailySchedule.diffRows(oldRows,newRows)` (LCS-based) — পুরনো বনাম প্রস্তাবিত rows-এর diff, ব্যবহৃত হয় তিন জায়গায় (teacher.html `renderSchedulePane()`, `renderChatScheduleStrip()`, student.html `renderTaskSchedulePanel()`) — বাদ দেওয়া সারি লাল স্ট্রাইকথ্রু (`.sched-diff-del`), নতুন/বদলানো সারি হলুদ হাইলাইট (`.sched-diff-add`, দুটোই `style.css`-এ)। ছাত্র এখন সরাসরি একটা সারি (✕ আইকন, `scheduleDeleteRow()`) বা পুরো সময়সূচি (`scheduleSubmitDeleteAll()`) মুছার প্রস্তাব পাঠাতে পারে — বিদ্যমান `submitProposal`/অনুমোদন pipeline-ই ব্যবহার হয় (খালি rows অ্যারে সাপোর্টেড)।
